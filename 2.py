...
def northwest_corner_method(cost_matrix, supply, demand):
    ...
    while source_idx < num_sources and dest_idx < num_destinations:
        ...
        if supply[source_idx] == 0:
            source_idx += 1
        if demand[dest_idx] == 0:
            dest_idx += 1
...
if d==1:
    allocation = northwest_corner_method(np.copy(cost_matrix), np.copy(supply), np.copy(demand))
else :
    allocation = northwest_corner_method(np.copy(cost_matrix2), np.copy(supply), np.copy(demand))
print("Initial  basic feasible solution using Northwest Corner Method:\n", allocation)
